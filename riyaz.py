...
ADMIN_IDS = [7915760293]  # Changed from string to list of integers
BOT_TOKEN = "7033927494:AAG3LmOwg2qE3_EWFTtBVrWYFa2puuGFkiE"
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By @RIYAZ_5U"  
...
def send_welcome(message):
    ...
    welcome_message = (f"Welcome, {username}! To @RIYAZ_5U\n\n"
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*You are not registered. Please redeem A key To Owner:- @RIYAZ_5U*", parse_mode='Markdown')
        return

    # Check if the user's key is still valid
    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*Your key has expired. Please redeem A key To Owner:- @RIYAZ_5U.*", parse_mode='Markdown')
        return
    ...
    if found_user:
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            account_info = ("Your key has expired. Please redeem a new key.\n"
                            "Contact @RIYAZ_5U for assistance.")
        else:
        ...
        account_info = "Please redeem A key To Owner:- @RIYAZ_5U."
